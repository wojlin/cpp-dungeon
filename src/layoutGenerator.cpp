...
int layoutGenerator::getRandomNumber(int minmumNumber, int maximumNumber)
{
...
void layoutGenerator::traverseBSP(BSP* bsp, nodeBSP* node)
{
...
std::vector<roomBox*> layoutGenerator::findRooms(nodeBSP* node)
{
...
int layoutGenerator::calculateMinimumNodeSize(int recursionsAmount, int desiredRoomSize)
{
    return static_cast<int>(pow(2, recursionsAmount)  * desiredRoomSize );
}

int layoutGenerator::calculateDesiredNodeSize(int dungeonDepth)
{
    return static_cast<int>(std::floor(sqrt(static_cast<float>(dungeonDepth)*2.0)+9.0));
}

int layoutGenerator::calculateRecursionsAmount(int dungeonDepth)
{
...
int layoutGenerator::calculateLevelSize(int recusionsAmount, int desiredNodeSize)
{   
    int min = 0;
...
BSP layoutGenerator::createBSP(int dungeonDepth)
{
...
void layoutGenerator::splitNodeBSP(nodeBSP* node, int desiredNodeSize)
{
...
std::string layoutGenerator::visualization(int x, int y, int w, int h)
{   
    std::string desmos;
...
//this will generate latex for demos: https://www.desmos.com/calculator/
std::string layoutGenerator::createVisualization(nodeBSP* node)
{   
    int w = node->width;
    int h = node->height;
    int x = node->posX;
...
//this will generate latex for demos: https://www.desmos.com/calculator/
std::string layoutGenerator::createVisualization(roomBox* room)
{   
    int w = room->width;
    int h = room->height;
    int x = room->posX;
...
std::string layoutGenerator::createVisualization(corridorLine* line)
{
...
roomBox layoutGenerator::createRoom(nodeBSP* node)
{
    roomBox room;
